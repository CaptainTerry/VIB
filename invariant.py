...
def mulitlayer_perceptron (input, envs, weights, bias):
    ...
    W5 = weights([int(n_hidden_3/2)+1, n_hidden_5], 'W5')
...
accuracy_env = tf.reduce_mean(tf.cast(tf.equal(tf.arg_max(pred_aware, 1), tf.arg_max(labels, 1)), tf.float32))
tf.summary.scalar('accuracy_en', accuracy_env)
...
for epoch in range(30):
    for step in range(int(steps_per_batch)):
        ...
        if path == 0:
            ...
            _, c1 = sess.run([gen_tensor, gen_loss], feed_dict={input: batch['input'], labels: batch['label'], envs: batch['env']})
        elif path in [1, 2, 3]:
            _, c2 = sess.run([env_inv_tensor, env_inv_loss], feed_dict={input: batch['input'], labels: batch['label'], envs: batch['env']})
        elif path in [4, 5, 6]:
            _, c3 = sess.run([env_enable_tensor, env_enable_loss], feed_dict={input: batch['input'], labels: batch['label'], envs: batch['env']})


    print("{}: gen_Test={:.2f}\t inv_Test={:.2f}\t acc_env={:.4f}\t acc_inv={:.4f} ".format(epoch, *evaluate_test()))
    sys.stdout.flush()
